                                        ...
                                        float sm_scale)
{
  ...
  // Preserve installed Sage's 64-row / per-thread quantization layout.
...
#pragma unroll
  ...
  float q_scales[num_tiles_q];
#pragma unroll
  for (uint32_t fq = 0; fq < num_tiles_q; fq++) {
    q_scales[fq] = q_tile_base + fq < q_scale_tiles ? Q_scale[q_scale_idx + fq * 32] : 0.0f;
  }
...
#pragma unroll
        for (uint32_t k = 0; k < 8; k++)
        {
          RS_f32[fq][fk][k] = __int2float_rz(RS[fq][fk][k]) * (q_scales[fq] * k_scale);
        }
      }
    }
...
void forward(torch::Tensor q,torch::Tensor k,torch::Tensor v,torch::Tensor o,
             torch::Tensor qs,torch::Tensor ks,torch::Tensor vs,float scale,int tile) {
  for (auto t : {q,k,v,o,qs,ks,vs}) {
    TORCH_CHECK(t.is_cuda() && t.device()==q.device(),"same CUDA device required");
    TORCH_CHECK(t.stride(-1)==1,"contiguous last dimension required");
  }
  c10::cuda::CUDAGuard guard(q.device());
  TORCH_CHECK(q.dim()==4 && k.dim()==4 && v.dim()==4 && o.dim()==4,"4D tensors required");
  TORCH_CHECK(q.scalar_type()==torch::kInt8 && k.scalar_type()==torch::kInt8,"INT8 Q/K required");
  TORCH_CHECK(v.scalar_type()==at::ScalarType::Float8_e4m3fn && o.scalar_type()==torch::kBFloat16,"FP8 V / BF16 output required");
  TORCH_CHECK(q.size(3)==128 && k.size(3)==128 && o.sizes()==q.sizes(),"head dimension 128 / matching output required");
  TORCH_CHECK(q.size(0)==k.size(0) && q.size(2)%k.size(2)==0 && k.size(1)>0,"invalid batch/heads/sequence");
  TORCH_CHECK(v.size(0)==k.size(0) && v.size(1)==128 && v.size(2)==k.size(2) && v.size(3)>=div_ceil(k.size(1),128)*128,"invalid padded V layout");
  for (auto t : {qs,ks,vs}) TORCH_CHECK(t.scalar_type()==torch::kFloat32 && t.is_contiguous(),"contiguous FP32 scales required");
  CHECK_SHAPE(qs,q.size(0),q.size(2),(int64_t)(div_ceil(q.size(1),64)*32));
  CHECK_SHAPE(ks,k.size(0),k.size(2),(int64_t)(div_ceil(k.size(1),128)*4));
